...
parser.add_argument('--val-idx-file', default='../index.npy', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if (not path.exists(args.val_idx_file)):
        val_idx = random.sample(all_idx, args.validation_size)
        mat = np.ones((args.validation_size, 2), dtype = np.int32)
        mat[:,1] = val_idx
    else:
        mat1 = np.load(args.val_idx_file)
        val_idx = random.sample(all_idx - set(mat1[:,1].flatten()), args.validation_size)
        mat2 = np.full((args.validation_size, 2), fill_value = np.max(mat1[:,0]) + 1, dtype = np.int32)
        mat2[:,1] = val_idx
        mat = np.concatenate([mat1, mat2])

    np.save(args.val_idx_file, mat)
    # np.save("val2_target.npy", np.asarray([train_dataset[x][1] for x in val_idx]))

    assert(args.validation_size == len(val_idx))
    validation_dataset = torch.utils.data.Subset(train_dataset, val_idx)

    train_subset = torch.utils.data.Subset(train_dataset,
            list(set(range(len(train_dataset))) - set(val_idx)))
...
def val2(validation_loader, model, epoch, args):
    ...
    np.save("val2_output{}.npy".format(epoch), prediction_array)

    if (not path.exists("val2_target.npy")):
        np.save("val2_target.npy", target_array)
    else:
        val2_load = np.load("val2_target.npy")
        assert(np.array_equal(val2_load, target_array))
    # np.save("train_target{}.npy".format(epoch), target_array)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    if (len(prediction_list) > 0):
        prediction_array = np.concatenate(prediction_list)
        target_array = np. concatenate(target_list)
        np.save("train_output{}.npy".format(epoch), prediction_array)
        np.save("train_target{}.npy".format(epoch), target_array)
...
def validate(epoch, val_loader, model, criterion, args):
    ...
    np.save("prob-e{epoch}.npy".format(epoch = epoch if epoch is not None else ''), prediction_array)
    ...
    if (not path.exists("validation_target.npy")):
        np.save("validation_target.npy", target_array)
    else:
        prev_array = np.load("validation_target.npy")
        assert np.array_equal(target_array, prev_array)

    # TODO: if the target file exists, check that the 
    # targets did not change between epochs
